...
function getTotalAtk(player) {
  ...
  // Title boost
  let titleAtk = 0;
  try { if (TitleSystem) { const tb = TitleSystem.getEquippedBoost(player); titleAtk = tb.atk || 0; } } catch(e) {}
  let atk = Math.floor((player.stats.atk + weapon + petAtk + titleAtk) * mods.atkMod);
...
function calculateAction(attacker, defender, action, atkState, defState, arena) {
  ...
  if (action.type === 'attack') {
    ...
    const defRed  = Math.floor((defender.stats.def || 0) * 0.4 * defMult);
...
async function executeBothActions(sock, chatId, p1, p2, db, save, sender) {
  ...
  let msg = `━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  if (arena) msg += `${arena.emoji} *${arena.name}*\n`;
  msg += `⚔️ *TURN ${turn} — BOTH STRIKE!*\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;
  if (arenaMsg) msg += arenaMsg + '\n';

  for (const [pl,res,dmgTaken,cls,rank,pid,state,wasStaggered] of ordered) {
    const hpAfter=pl.stats.hp; const hpPct=hpAfter/pl.stats.maxHp; const threat=getThreatIcon(hpPct);
    msg += `${rank.emoji} *${pl.name}* [${cls}]:\n`;
    msg += res.narrative;
    if (dmgTaken > 0 && !res.wasParried) msg += `🩹 Took *${dmgTaken}* damage!\n`;
    if (wasStaggered) msg += `⭐ *STAGGERED!* ${pl.name} stunned next turn!\n`;
    if (state.rageMode && hpPct<=0.20) msg += `🔥 *RAGE MODE!* +30% ATK\n`;
    if (state.ultiReady) msg += `🟣 *ULTIMATE READY!* → /pvp ultimate\n`;
    if (state.conquerorMode>0) msg += `👑 *CONQUEROR MODE!* ×3 ATK — ${state.conquerorMode} turns left!\n`;
    if (state.holyStrikes>0) msg += `✨ *HOLY STRIKES:* ${state.holyStrikes} charges!\n`;
    msg += `\n`;
  }
  ...
  if (p1s.skillCooldowns?.some(cd=>cd>0)) msg+=`🔒 *Skills:* ${getCdBar(p1s.skillCooldowns)}\n`;

  msg += `\n${getThreatIcon(p2.stats.hp/p2.stats.maxHp)} *${p2.name}*\n`;
  ...
  if (p2s.skillCooldowns?.some(cd=>cd>0)) msg+=`🔒 *Skills:* ${getCdBar(p2s.skillCooldowns)}\n`;
  msg += `━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;

  // Desperation unlock hints
  const p1HpPct=p1.stats.hp/p1.stats.maxHp;
  const p2HpPct=p2.stats.hp/p2.stats.maxHp;
  if (p1HpPct<=0.15 && !p1s.desperationUsed) msg+=`⚠️ @${p1Id.split('@')[0]} *DESPERATION UNLOCKED!* → /pvp desperation\n`;
  if (p2HpPct<=0.15 && !p2s.desperationUsed) msg+=`⚠️ @${p2Id.split('@')[0]} *DESPERATION UNLOCKED!* → /pvp desperation\n`;
...
async function handleVictory(sock, chatId, winner, loser, wId, lId, db, save, wStats, lStats, totalTurns) {
  ...
  const xpNeeded = Math.floor(100 * Math.pow(1.5, winner.level - 1));

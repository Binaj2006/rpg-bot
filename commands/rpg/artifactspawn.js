...
async function spawnArtifact(sock, chatId, db, saveDatabase, forcedArtifact) {
  ...
  setTimeout(() => {
    ...
    if (spawn && !spawn.claimed) {
      ...
      sock.sendMessage(chatId, { text: `⌛ *${artifact.emoji} ${artifact.name}* faded away...\n💭 No one was fast enough to claim it.\n\n⏰ Next artifact spawns in 2-3 hours!` }).catch(() => {});
    }
  }, 5 * 60 * 1000);
}
...
// CLAIM COMMAND
// ═══════════════════════════════════════════════════════════════
async function handleClaim(sock, msg, args, db, saveDatabase, sender) {
  ...
  if (spawn.claimed) {
    ...
    return sock.sendMessage(chatId, { text: `❌ Already claimed by *${winner?.name || 'someone'}*!\n⏰ Next spawn in 2-3 hours.` }, { quoted: msg });
  }
